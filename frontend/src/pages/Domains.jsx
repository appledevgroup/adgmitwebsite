...
import { FaApple, FaAndroid, FaGlobe, FaBrain } from 'react-icons/fa'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto items-start">
          {domains.map((domain, i) => (
            <motion.div
              ...
              className={`p-8 rounded-xl shadow-lg text-left self-start min-h-[9rem] transition-colors duration-300 ${expanded === i ? 'bg-[#1d1d1d] text-white border border-gray-600' : 'bg-[#1d1d1d] text-white'}`}>
              ...
              >
                <p className="text-white text-center">{domain.description}</p>
              </motion.div>
            </motion.div>
          ))}
        </div>
